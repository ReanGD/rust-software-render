...
impl ModelObj {
    fn get_index(&mut self, index: IndexTuple) -> u32 {
        ...
        let len = self.model.vertex_buffer.len() as u32;
        self.map.entry(index).or_insert(len).clone()
    }

    fn parse(&mut self) -> Result<(), String> {
        for object in self.model_obj.object_iter() {
    ...
    pub fn load(filename: &str) -> Result<Model, String> {
        ...
        let mut model_dir = std::path::PathBuf::from(filepath);
        if !model_dir.pop() {
